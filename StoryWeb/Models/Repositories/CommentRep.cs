...
namespace StoryWeb.Models.Repositories
{
    public class CommentRep
    {
        ...
        public async Task<List<ChapterComment>> GetChapterCommentsAsync(int chapterId)
        {
            try
            {
                HttpClient client = new HttpClient();
                ...
                HttpResponseMessage res = await client.GetAsync($"api/comment/chapter/{chapterId}");
                if (res.IsSuccessStatusCode)
                {
                    var json = await res.Content.ReadAsStringAsync();
                    return JsonConvert.DeserializeObject<List<ChapterComment>>(json);
                }
            }
            catch (Exception ex)
            {
            ...
            return new List<ChapterComment>();
        }
    }
}